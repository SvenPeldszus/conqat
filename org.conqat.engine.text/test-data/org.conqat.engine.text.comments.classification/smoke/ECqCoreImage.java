...
public enum ECqCoreImage {
	...
	/** New Bundle Wizard §interface§ */
	NEW_BUNDLE_WIZARD,

	/** Bundle icon §interface§ */
	BUNDLE,

	/** New bundle icon (this is only referenced from plugin.xml) §interface§ */
	NEW_BUNDLE;
	...
	/** Get image registry from {@link CqCoreActivator}. §interface§ */
	private static ImageRegistry getRegistry() {
		return CqCoreActivator.getDefault().getImageRegistry();
	}

	/** Get image for enumeration element. §interface§ */
	public Image getImage() {
		return getRegistry().get(getImageName());
	}

	/** Get image descriptor for enumeration element. §interface§ */
	public ImageDescriptor getImageDescriptor() {
		return CqCoreActivator.getImageDescriptor(getImagePath());
	}

	/** Get image name. §interface§ */
	private String getImageName() {
		return name().toLowerCase();
	}

	/** Get path to image. §interface§ */
