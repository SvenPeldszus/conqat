 ...
 * @version $Revision: 48732 $
 * @ConQAT.Rating GREEN Hash: D807AC80C829DD224DB505CBB45A9669
 */
public class CompoundBreaker {
	...
	public static List<String> breakCompound(String compound) {
		return breakCompound(compound, true);
	}

	/**
	 * Break compound word and return the components of the compound.
	 * 
	 * @param discardShortParts
	 *            if this is true, parts of length 1 are discarded.
	 */
	public static List<String> breakCompound(String compound,
			boolean discardShortParts) {
		...
		for (String part : compound.split(UNDERSCORE)) {
			...
			if (!discardShortParts || part.length() >= 2) {
				result.add(part);
			}
		}
		return result;
