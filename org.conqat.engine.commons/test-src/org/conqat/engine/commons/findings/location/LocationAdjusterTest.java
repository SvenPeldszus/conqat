 ...
 * @version $Rev: 50376 $
 * @ConQAT.Rating GREEN Hash: E19DFA5CF74ABA67FE8F86A84F858E83
 */
public class LocationAdjusterTest extends CCSMTestCaseBase {
	...
	/** Tests adjustment of single lines. */
	public void testLineAdjustment() throws ConQATException {
		String originalString = "abc\ndef ghi\njkl";

		// no change, so same line
		assertEquals("[2-2]", new LocationAdjuster(originalString,
				originalString).adjustLine(2).toString());

		// insert empty line
		assertEquals("[3-3]", new LocationAdjuster(originalString,
				"abc\n \n def ghi\njkl").adjustLine(2).toString());

		// split line, so result has two lines
		assertEquals("[2-3]", new LocationAdjuster(originalString,
				"abc\ndef\nghi\njkl").adjustLine(2).toString());

		// merge line, but result is still one line
		assertEquals("[2-2]", new LocationAdjuster(originalString,
				"abc\ndef ghi jkl").adjustLine(2).toString());

		// delete line, expect null
		assertEquals(null,
				new LocationAdjuster(originalString, "abc\njkl").adjustLine(2));
