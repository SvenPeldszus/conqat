 ...
 * @version $Rev: 49717 $
 * @ConQAT.Rating GREEN Hash: ED6F4FC12F13CCFFD092FC5D44D8C5C5
 */
public abstract class ConQATCommonsProcessorTestCaseBase extends
	...
	/**
	 * Returns a list of findings that is sorted in a stable fashion. Basically,
	 * this just sorts by location string.
	 */
	protected static List<Finding> sortFindings(Collection<Finding> findings) {
		return CollectionUtils.sort(findings, new Comparator<Finding>() {

			@Override
			public int compare(Finding finding1, Finding finding2) {
				return finding1.getLocationString().compareTo(
	...
	/**
	 * Extracts the findings from the key "findings" and returns them as a
	 * string that given the string representation of the sorted findings list.
	 */
	protected static String extractFindingsAsString(IConQATNode node) {
		return extractFindingsAsString(node, "findings");
	}

	/**
	 * Extracts the findings from the given key and returns them as a string
	 * that given the string representation of the sorted findings list. The
	 * individual findings will be in separate lines, separated by '\n' (not the
	 * platform separator).
	 */
	protected static String extractFindingsAsString(IConQATNode node, String key) {
		FindingsList findingsList = (FindingsList) node.getValue(key);
		if (findingsList == null) {
			return StringUtils.EMPTY_STRING;
		}
		List<String> findingsStrings = new ArrayList<>();
		for (Finding finding : sortFindings(findingsList)) {
			findingsStrings.add(finding.getMessage() + " @ "
					+ finding.getLocationString());
		}
		return StringUtils.concat(findingsStrings, "\n");
	}
