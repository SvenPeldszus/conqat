 ...
 * @version $Rev: 50089 $
 * @ConQAT.Rating GREEN Hash: 6AEFC9506B5DA5ADE75B1E1A417E937E
 */
public abstract class ProcessExecutorBase extends ConQATProcessorBase {
	...
	private void execute() throws IOException, ConQATException {
		...
		if (failOnError && exitValue != getSuccessExitCode()) {
			throwProcessFailure(stdOut, stdErr, exitValue);
		}

	}

	/**
	 * Throw a {@link ConQATException} with information about the cause of the
	 * process failure, i.e. std error and std out of the execution.
	 */
	private void throwProcessFailure(String stdOut, String stdErr, int exitValue)
			throws ConQATException {
		StringBuilder message = new StringBuilder();
		message.append("Process failed with exit value " + exitValue);
		message.append(StringUtils.CR);
		message.append("Standard Error:");
		message.append(StringUtils.CR);
		message.append(stdErr);
		message.append(StringUtils.CR);
		message.append("Standard Out:");
		message.append(StringUtils.CR);
		message.append(stdOut);
		throw new ConQATException(message.toString());
	}
