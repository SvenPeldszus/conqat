 ...
 * @version $Rev: 50585 $
 * @ConQAT.Rating YELLOW Hash: ADC42969C1C12ABAF53FA3326ACBAEEC
 */
public abstract class ConQATProcessorTestCaseBase extends CCSMTestCaseBase {
	...
	/** @see #executeProcessor(Class, IConQATLogger, Object...) */
	protected Object executeProcessor(
			Class<? extends IConQATProcessor> processorClass, Object... args)
			throws ConQATException {
		return executeProcessor(processorClass, new LoggerMock(), args);
	}
	 ...
	 * @param logger
	 *            The ConQATLogger to catch log messages.
	 * @param args
	...
	protected Object executeProcessor(
			...
			IConQATLogger logger, Object... args) throws ConQATException {

		try {
			return CQDDLUtils.executeProcessor(processorClass.getName(),
					logger, parsingParameters, args);
	...
	/** Creates the given bundle context. */
	protected void createBundleContext(
			Class<? extends BundleContextBase> bundleContextClass)
			throws BundleException {
		File bundleDirectory = getBundleDir(bundleContextClass);
		try {
			bundleContextClass.getConstructor(BundleInfo.class).newInstance(
					new BundleInfo(bundleDirectory));
		} catch (InstantiationException | IllegalAccessException
				| IllegalArgumentException | InvocationTargetException
				| NoSuchMethodException | SecurityException e) {
			throw new AssertionError("Failed to create bundle context "
					+ bundleContextClass, e);
		}
	}
	...
	protected static File getBundleDir(Class<?> clazz) {
		...
		String classesDir = StringUtils.stripSuffix(
				classFile, clazz.getName().replace('.', '/') + ".class");
		String bundleDir = StringUtils.stripSuffix(classesDir, "classes/");
