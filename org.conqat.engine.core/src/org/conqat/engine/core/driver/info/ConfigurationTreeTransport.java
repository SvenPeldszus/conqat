 ...
 * @author $Author: goede $
 * @version $Rev: 47434 $
 * @ConQAT.Rating GREEN Hash: C50C5D5C5B49435DBCECBBEABC38DF47
 */
public class ConfigurationTreeTransport implements Serializable {
	...
	/**
	 * The XML meta-data stored in the cq.edit key. We persist this as a string,
	 * to ensure clean transport via RMI.
	 */
	...
	public ConfigurationTreeTransport(IInfo info) {
		...
		if (info instanceof BlockInfo) {
			block = true;
			cqEditMeta = serializeElement(((BlockInfo) info)
	...
	/** Serializes the given element to a string. */
	private String serializeElement(Element metaElement) {
		try {
			Document document = DocumentBuilderFactory.newInstance()
					.newDocumentBuilder().newDocument();
			document.adoptNode(metaElement.cloneNode(true));
	...
	public Element getCQEditMeta() {
		...
		try {
			Document document = XMLUtils.parse(new InputSource(
					new StringReader(cqEditMeta)));
