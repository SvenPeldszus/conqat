 ...
 * @version $Rev: 50372 $
 * @ConQAT.Rating GREEN Hash: 2DBB2955103F09F2A0D1E65AC7DDFA1E
 */
public class ExecutionContext {
	...
	private CanonicalFile createUniqueFile(File baseDir, String prefix,
		...
		File result = new File(baseDir, prefix + suffix);

		// We use a loop to determine the filename, as we never know if a
		// processor creates additional files with colliding names.
		while (result.exists()) {
	...
	/** Executes the shutdown hooks and optionally removes the temporary files. */
	public void performShutdown(boolean deleteTemporaryFiles) {
