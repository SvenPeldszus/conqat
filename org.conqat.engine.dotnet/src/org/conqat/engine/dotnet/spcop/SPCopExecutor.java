 ...
 * @version $Rev: 50585 $
 * @ConQAT.Rating YELLOW Hash: 81FC552A39B4E03E744B9AF2A213D53A
 */
...
public class SPCopExecutor extends DotnetExecutorBase {
	...
	private String getWSPFilePath(IElement element) throws ConQATException {
		if (element.getLocation().toLowerCase().contains(".zip!")) {
			try {
				String baseFileName = UniformPathUtils.getElementName(element.getUniformPath());
				baseFileName = StringUtils.stripSuffix(baseFileName, ".wsp");
				CanonicalFile wspFile = getProcessorInfo().getTempFile(baseFileName,
						".wsp");
	...
	private IContentAccessor readGeneratedReport(CanonicalFile report)
		...
		String modifiedPath = StringUtils.stripSuffix(originalPath, ".xml")
				+ "_Rules.xml";
		try {
