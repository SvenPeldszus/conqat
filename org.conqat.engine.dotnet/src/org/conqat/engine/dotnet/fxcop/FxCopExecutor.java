 ...
 * @version $Rev: 50790 $
 * @ConQAT.Rating YELLOW Hash: DF4B26598D6DF6B680F8525A91699D4D
 */
...
public class FxCopExecutor extends DotnetExecutorBase {
	...
	/** {@ConQAT.Doc} */
	@AConQATFieldParameter(parameter = "structured-logging", attribute = "enable", description = "If true errors will be logged using structured log messages. Default: false", optional = true)
	public boolean useStructuredLogging = false;
	...
	/**
	 * Logs the error with the configured error level for the given collection,
	...
	private void logIfNotEmpty(Collection<String> errors, String formatMessage) {
		...
		String message = String.format(formatMessage, errors.size());

		Object logMessage;
		if (useStructuredLogging) {
			logMessage = new ListStructuredLogMessage(message, errors,
					StructuredLogTags.THIRD_PARTY_TOOL);
		} else {
			logMessage = message + ":" + StringUtils.CR
					+ StringUtils.concat(errors, StringUtils.CR);
		}

		getLogger().log(errorLogLevel, logMessage);
	}
	...
	private Map<CanonicalFile, IElement> extractAnalysisAssemblies()
			throws ConQATException {
		CanonicalFile tempDirectory = createTempDirectory("Assemblies");
	...
	private CanonicalFile extractDependencyAssemblies() throws ConQATException {
		...
		CanonicalFile tempDirectory = createTempDirectory("Dependencies");
		extractAssemblies(dependencyScope, tempDirectory, false);
		return tempDirectory;
	}

	/**
	 * Creates a temporary directory or throws an {@link ConQATException} if not
	 * possible. The newly created directory is returned as
	 * {@link CanonicalFile}.
	 */
	private CanonicalFile createTempDirectory(String folderSuffix)
			throws ConQATException {
		CanonicalFile tempDirectory = getProcessorInfo().getTempFile(
				TEMP_FILE_PREFIX, folderSuffix);
		if (!tempDirectory.mkdir()) {
			throw new ConQATException("Could not create temp directory "
					+ tempDirectory.getCanonicalPath());
		}
		return tempDirectory;
	}
