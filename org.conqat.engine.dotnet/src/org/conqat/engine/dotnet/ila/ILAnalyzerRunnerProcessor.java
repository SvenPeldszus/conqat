/*--------------------------------------------------------------------------+
$Id: ILAnalyzerRunnerProcessor.java 50267 2014-08-22 14:06:50Z poehlmann $
|                                                                          |
 ...
 * @version $Revision: 50267 $
 * @ConQAT.Rating GREEN Hash: F8D8E1C8AEAE2E9214B42D9ABADF5D71
 */
...
public class ILAnalyzerRunnerProcessor extends DotnetExecutorBase {
	...
	/**
	 * Controls whether the ILA should write member body information to the XML.
	 */
	private boolean excludeMembers = true;

	/** Controls whether the ILA should write dependency information to the XML. */
	private boolean excludeDependencies = false;

	/** {@ConQAT.Doc} */
	@AConQATParameter(name = "exclude", minOccurrences = 0, maxOccurrences = 1, description = ""
			+ "Toggles ILA options to exclude certain elements from the XML")
	public void setExcludeOptions(
			@AConQATAttribute(name = "members", defaultValue = "true", description = ""
					+ "If set, ILA does not include member body information into the XML, which is not "
					+ "required for architecture analysis. Default is true.") boolean excludeMembers,
			@AConQATAttribute(name = "dependencies", defaultValue = "false", description = ""
					+ "If set, ILA does not include dependency information into the XML, which is not "
					+ "required for some analyses. This reduces the XML size by about one third. "
					+ "Default is false.") boolean excludeDependencies) {
	...
	/** Processes the input by running the ILAnanlyzer on the input binaries. */
	@Override
	...
	private List<String> createCommandLineArguments(File file) {
		...
		if (excludeDependencies) {
			arguments.add("-excludeDependencies");
		}
