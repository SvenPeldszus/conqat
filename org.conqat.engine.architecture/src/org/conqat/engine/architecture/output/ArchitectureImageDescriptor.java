 ...
 * @version $Rev: 50518 $
 * @ConQAT.Rating GREEN Hash: 7E5A7465E8028DA5BFD025DA5F04BA0B
 */
public class ArchitectureImageDescriptor extends ImageDescriptorBase {
	...
	/** Constructor. */
	public ArchitectureImageDescriptor(ArchitectureDefinition architecture,
	...
	public void draw(Graphics2D graphics, int width, int height) {
		...
		for (ComponentNode component : architecture.getChildren()) {
	...
	private String getLabel(ComponentNode component) {
		String componentName = StringUtils.getLastPart(component.getName(),
				ArchitectureDesignConstants.ARCHITECTURE_NAME_SEPARATOR);
	...
	private String stripName(String name, FontMetrics metrics, int width) {
		...
		return abbreviateName(name, metrics, width);
	}

	/** Abbreviate the name until it fits the vertex. */
	private String abbreviateName(String name, FontMetrics metrics, int width) {
		// cannot abbreviate
		...
		return abbreviateName(name.substring(0, name.length() - 1), metrics,
				width);
	}
	...
	private void paintEdges(Graphics2D graphics) {
		List<DependencyPolicy> edges = architecture.getSortedPolicies();
	...
	private void paintEdge(Graphics2D graphics, DependencyPolicy edge) {
		...
		List<Point> innerPoints = edge.getPoints();
		...
		if (!innerPoints.isEmpty()) {
			startPoint = getChopboxAnchor(edge.getSource(), innerPoints.get(0));
			endPoint = getChopboxAnchor(edge.getTarget(),
					CollectionUtils.getLast(innerPoints));
		}

		for (Point innerPoint : innerPoints) {
			graphics.drawLine(startPoint.x, startPoint.y, innerPoint.x,
					innerPoint.y);
			startPoint = innerPoint;
		}

		paintEdgeLastSegmentAndArrow(graphics, edge, startPoint, endPoint,
				innerPoints);
	}

	/** Paints the last segment and the arrow of an edge. */
	private void paintEdgeLastSegmentAndArrow(Graphics2D graphics,
			DependencyPolicy edge, Point startPoint, Point endPoint,
	 ...
	 * a line drawn from the center point of the component to the center of a
	 * reference component.
	 */
	private Point getChopboxAnchor(ComponentNode baseNode,
		...
		return getChopboxAnchor(baseNode, referencePoint);
	}

	/**
	 * The ChopboxAnchor's location is found by calculating the intersection of
	 * a line drawn from the center point of the component to a reference point
	 * on that box.
	 */
	private Point getChopboxAnchor(ComponentNode baseNode, Point referencePoint) {
		Rectangle rectangle = baseNode.getAbsoluteBounds();
		rectangle.grow(1, 1);
		return GraphicsUtils.getChopboxAnchor(rectangle, referencePoint);
	}
	...
	private Rectangle determineImageBounds() {
		...
		for (IPolicy edge : architecture.getAllPolicies()) {
			if (!(edge instanceof DependencyPolicy)) {
				continue;
			}
			for (Point point : ((DependencyPolicy) edge).getPoints()) {
	...
	public Object getSummary() {
		return NodeUtils.getSummary(architecture);
	}
	...
	private class TooltipDescriptor implements
		...
		public DisplayList getDisplayList() {
			return NodeUtils.getDisplayList(architecture);
		}
