 ...
 * @version $Rev: 50164 $
 * @ConQAT.Rating GREEN Hash: 00ABF87B6297770313E5977B521AD7DA
 */
public class TypeToComponentMapper {
	 ...
	 * names.
	 * */
	private final ListMap<String, IComponent> typesToComponents = new ListMap<String, IComponent>();

	/** Maps from a component to the types mapped to this component. */
	private final SetMap<IComponent, String> componentsToTypes = new SetMap<IComponent, String>();
	...
	private final Comparator<IComponent> componentByNameComparator = new Comparator<IComponent>() {
		@Override
	...
	public void map(Set<String> types) {
		...
		Pattern scopeInclude = Pattern.compile(".*");
		if (!StringUtils.isEmpty(architecture.getScopeIncludePattern())) {
			try {
				scopeInclude = Pattern.compile(architecture
						.getScopeIncludePattern());
			} catch (PatternSyntaxException e) {
				CCSMAssert
						.fail("Expect only valid regexes in model, as editor performs checking during editing. "
								+ "Failed pattern: "
								+ architecture.getScopeIncludePattern());
			}
		}

		Pattern scopeExclude = Pattern.compile(StringUtils.EMPTY_STRING);
		try {
			scopeExclude = Pattern.compile(architecture
					.getScopeExcludePattern());
		} catch (PatternSyntaxException e) {
			CCSMAssert
					.fail("Expect only valid regexes in model, as editor performs checking during editing. "
							+ "Failed pattern: "
							+ architecture.getScopeExcludePattern());
		}

		mapTypes(types, components, scopeInclude, scopeExclude);
	}

	/** Maps types to components. */
	private void mapTypes(Set<String> types,
			List<? extends IComponent> components, Pattern scopeInclude,
			Pattern scopeExclude) {
		int typesProcessed = 0;
		for (String type : types) {
			if (isInArchitectureScope(type, scopeInclude, scopeExclude)) {
				List<IComponent> matchingComponents = getMatchingComponents(
						type, components);
				if (matchingComponents.isEmpty()) {
					orphans.add(type);
				} else {
					Collections.sort(matchingComponents,
							componentByNameComparator);
	...
	/**
	 * Returns whether the given type is contained in the current scope of the
	 * architecture.
	 */
	private boolean isInArchitectureScope(String type, Pattern scopeInclude,
			Pattern scopeExclude) {
		return scopeInclude.matcher(type).matches()
				&& !scopeExclude.matcher(type).matches();
	}

	/**
	 * Returns the names of all types without associated component (a.k.a.
	 * orphans).
	 */
	...
	public IComponent getMappedComponentForType(String type) {
		List<IComponent> mappedComponents = typesToComponents
				.getCollection(type);
	...
	public ListMap<String, IComponent> getTypesMatchingMultipleComponents() {
		ListMap<String, IComponent> typesMatchingMultipleComponents = new ListMap<String, IComponent>();
		for (String type : typesToComponents.getKeys()) {
			List<IComponent> matchingComponents = typesToComponents
					.getCollection(type);
	...
	private List<? extends IComponent> getAllComponentsSortedByDepth() {
		List<? extends IComponent> components = CollectionUtils.sort(
				architecture.getAllComponents(), new Comparator<IComponent>() {
					@Override
					public int compare(IComponent a, IComponent b) {
	 ...
	 */
	private boolean isMappedTo(String type, IComponent component) {
