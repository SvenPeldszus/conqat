 ...
 * @version $Rev: 49590 $
 * @ConQAT.Rating GREEN Hash: 8F800995A792F30DEC42D9AC4CE9E394
 */
...
public class NamingConventionAnalyzerTest extends TokenTestCaseBase {
	...
	public void testTypes() throws ConQATException {
		runTest("type-names", finding(7, "C1"), finding(46, "Test"),
				finding(52, "thisStruct"), finding(52, "myStruct"));
	}
	...
	public void testAttributes() throws ConQATException {
		runTest("attribute-names", finding(12, "attribute1"), finding(53, "i"));
	}
	...
	/** Test constant with fully qualified type name in Java. */
	public void testJavaConstant() throws Exception {
		ITokenElement element = createTokenElement("public class A { static final org.apache.log4j.Logger LOGGER; }");
		executeProcessor(NamingConventionAnalyzer.class, "(input=(ref=",
				element, "), 'constant-names'=(pattern='x'))");
		assertOneFinding(element,
				"Name violates naming convention: LOGGER. Should be one of 'x'.");
	}

	/** Tests with array initializer (CR#6503). */
	public void testArrayInitializer() throws Exception {
		ITokenElement element = createTokenElement("public class A { static final int[] CONST = { 1, 2, glob, BAR.foo }; }");
		executeProcessor(NamingConventionAnalyzer.class, "(input=(ref=",
				element, "), 'constant-names'=(pattern='[A-Z]+'))");
	...
	/** Tests C# namespaces (CR#5981). */
	public void testCsNamespace() throws Exception {
		ITokenElement element = createTokenElement(
				"namespace foo { public class Bar { } }", ELanguage.CS);
		executeProcessor(NamingConventionAnalyzer.class, "(input=(ref=",
				element, "), 'module-names'=(pattern='x'))");

		assertOneFinding(element,
				"Name violates naming convention: foo. Should be one of 'x'.");
	}

	/**
	 * Asserts that the given element has exactly one finding with given
	 * message.
	 */
	private void assertOneFinding(ITokenElement element, String message) {
		FindingsList findings = NodeUtils.getOrCreateFindingsList(element,
				ElementFindingAnalyzerBase.DEFAULT_KEY);
		assertEquals(1, findings.size());
		assertEquals(message, findings.get(0).getMessage());
	}
