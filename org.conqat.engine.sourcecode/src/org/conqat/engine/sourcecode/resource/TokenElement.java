 ...
 * @version $Rev: 50872 $
 * @ConQAT.Rating GREEN Hash: 19A69FAB8730136A589A871F7D8E2274
 */
public class TokenElement extends TextElement implements ITokenElement {

	/**
	 * Global flag that can be used to disable logging of scanner warnings in a
	 * context where scanner warnings are handled elsewhere.
	 */
	public static boolean logScannerWarnings = true;

	/** The cache used for storing tokens. */
	@SuppressWarnings("unchecked")
	private static final ICache<ObtainTokensKey, UnmodifiableList<IToken>, ConQATException> TOKEN_CACHE = CacheFactory
	...
	public UnmodifiableList<IToken> getTokens(IConQATLogger logger)
		...
		return TOKEN_CACHE.obtain(new ObtainTokensKey(this, logger));
	}
	...
	private List<IToken> obtainValidTokens(List<IToken> allTokens,
		...
		for (IToken token : allTokens) {
			if (token.getType().isError()) {
				if (logScannerWarnings && logMessageCount < NUM_OF_SCANNER_WARNINGS) {
