 ...
 * @version $Rev: 47945 $
 * @ConQAT.Rating GREEN Hash: F2955FF040031326976D2AF553E1DA21
 */
public class TokenElementUtils {
	 ...
	 * in the same line. For the very first token, the empty string is returned.
	 */
	private static String determineSeperator(IToken token, IToken lastToken) {
		...
		int lastTokenLastLine = lastToken.getLineNumber()
				+ StringUtils.countLines(lastToken.getText()) - 1;

		// continued on same line
		if (token.getLineNumber() == lastTokenLastLine) {
