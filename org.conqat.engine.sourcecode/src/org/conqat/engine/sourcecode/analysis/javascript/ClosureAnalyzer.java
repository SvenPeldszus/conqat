 ...
 * @version $Rev: 47368 $
 * @ConQAT.Rating GREEN Hash: DFEDE9870179E711195727BD0B337050
 */
...
public class ClosureAnalyzer extends ConQATPipelineProcessorBase<ITextResource> {
	...
	protected void processInput(ITextResource input) throws ConQATException {
		...
		compiler.compile(new ArrayList<SourceFile>(),
				determineInputFiles(input), determineOptions());
	...
	private List<SourceFile> determineInputFiles(ITextResource input)
			throws ConQATException {
		List<SourceFile> inputFiles = new ArrayList<SourceFile>();
		for (ITextElement element : ResourceTraversalUtils
				.listTextElements(input)) {
			inputFiles.add(SourceFile.fromCode(element.getUniformPath(),
