 ...
 * @version $Rev: 50371 $
 * @ConQAT.Rating RED Hash: 874DAF7EF05E2477A7C2A3197373E566
 */
public class ShallowEntity {
	...
	/**
	 * Returns a new list containing all children of the given subtype in order.
	 */
	// TODO (BH): I think the interface is strange. Shouldn't you also provide a
	// type in addition to the subtype? Only a subtype seems to be potentially
	// dangerous.
	public List<ShallowEntity> getChildrenOfSubType(String subtype) {
		List<ShallowEntity> result = new ArrayList<ShallowEntity>();
		for (ShallowEntity child : children) {
			// TODO (BH): Never compare strings with == (unless you really know
			// what you do)
			if (child.getSubtype() == subtype) {
				result.add(child);
			}
		}
		return result;
	...
	// TODO (BH): Why has this been changed to public. IMHO this should be kept package visible!
	/* package */public void setEndTokenIndex(int endTokenIndex) {
	 ...
	 * tokens without comments. This may not be the correct index, if parsing
	 * errors occurred.
	 */
	 ...
	 * non-empty, but may be empty in very special cases, such as parsing files
	 * with invalid syntax. For such files, this may even be empty although
	 * child token lists are non-empty.
	 */
	...
	/**
	 * Returns whether this is an empty entity, i.e. does not contain any
	 * tokens. An entity is empty only in very special cases, such as parsing
	...
	public UnmodifiableList<IToken> ownEndTokens() {
		...
		return readOnlyTokenView(
				CollectionUtils.getLast(children).endTokenIndex, endTokenIndex);
	}
	...
	/**
	 * Returns a string representation of this node without including its
	 * children but using unfiltered lines. The end line might be output as -1
	 * if the node is incomplete.
	 */
	public String toLocalStringUnfiltered(ITextElement element)
		...
		int endLine = -1;
		if (endToken != null) {
			endLine = TextElementUtils.convertFilteredOffsetToUnfilteredLine(
					element, endToken.getEndOffset());
		}
		return toLocalString(
				TextElementUtils.convertFilteredOffsetToUnfilteredLine(element,
						getStartToken().getOffset()), endLine);
	}
