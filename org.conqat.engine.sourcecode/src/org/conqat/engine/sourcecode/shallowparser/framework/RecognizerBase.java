 ...
 * @version $Rev: 51370 $
 * @ConQAT.Rating GREEN Hash: 159B59AE5CC0BFF7DECA208E9FF5C767
 */
public abstract class RecognizerBase<STATE extends Enum<STATE>> {
	...
	/**
	 * The recognizers that are to be executed after this one. If there are more
	...
	public RecognizerBase<STATE> skipNested(ETokenType open, ETokenType close) {
		return skipNested(open, close, null);
	}

	/**
	 * Skips a region consisting of nested elements if it exists (i.e., the
	 * skipping is optional). The skipped region will be processed by the given
	 * subRecognizer (may be null), which can optionally create further
	 * entities.
	 */
	public RecognizerBase<STATE> skipNested(ETokenType open, ETokenType close,
		...
		return appendRecognizer(new OptionalNestedRecognizer<STATE>(open,
				close, subRecognizer));
	}
	...
	public RecognizerBase<STATE> skipToWithNesting(Object matchTerm,
		...
		return skipToWithNesting(matchTerm, open, close, null);
	}

	/**
	 * Skips all tokens up to the given match term. If the match term occurs
	 * within a nested structure, it is ignored. The skipped region will be
	 * processed by the given subRecognizer (may be null), which can optionally
	...
	public RecognizerBase<STATE> skipBeforeWithNesting(Object matchTerm,
			...
			ETokenType close2) {
		return skipBeforeWithNesting(matchTerm, open1, close1, open2, close2,
				null);
	}
	 ...
	 * Skips all tokens up to the given match term. If the match term occurs
	 * within a nested structure, it is ignored. The new position is set
	 * directly before the found match. The skipped region will be processed by
	 * the given subRecognizer (may be null), which can optionally create
	 * further entities.
	 */
	public RecognizerBase<STATE> skipBeforeWithNesting(Object matchTerm,
			ETokenType open1, ETokenType close1, ETokenType open2,
			ETokenType close2, RecognizerBase<STATE> subRecognizer) {
		SkipToRecognizer<STATE> skipper = new SkipToRecognizer<STATE>(
				new Object[] { matchTerm }, true);
	...
	/**
	 * Creates a shallow parser entity node, offsetting the node start by the
	 * given amount of tokens to the right.
	 * 
	 * @param subtype
	 *            this may be either a string which is used directly, or an
	 *            integer which is interpreted as an offset into the tokens
	 ...
	 *            the start and end values define a range of indexes.
	 * @param offset
	 *            an offset in the token stream. Moves the node start that many
	 *            tokens to the right.
	 */
	public RecognizerBase<STATE> createNode(EShallowEntityType type,
			Object subtype, Object name, int offset) {
		return appendRecognizer(new CreateNodeRecognizer<STATE>(type, subtype,
				name, offset));
	}
	 ...
	 * This can be used for long constructs where the name is found at the end.
	 */
	public void endNodeWithName(final Object name) {
		appendRecognizer(new RecognizerBase<STATE>() {
			@Override
			public int matches(ParserState<STATE> parserState,
					List<IToken> tokens, int startOffset) {
				parserState.setNodeName(RecognizerUtils.resolveName(tokens,
						parserState, startOffset, name));
				parserState.endNode(false);
				return startOffset;
			}
		});
