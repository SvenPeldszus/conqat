 ...
 * @version $Rev: 51186 $
 * @ConQAT.Rating GREEN Hash: 47869FC1AC0CB98F5E9879832489E84F
 */
public class JavaShallowParser extends CStyleShallowParserBase {
	...
	protected void createMetaRules() {
		// imports and package
		inAnyState().markStart().sequence(EnumSet.of(IMPORT, PACKAGE))
				.skipTo(SEMICOLON)
	...
	protected RecognizerBase<EGenericParserStates> typePattern(
		...
		return currentState.repeated(modifierKeywords).skipNested(LT, GT)
				.repeated(IDENTIFIER, DOT).optional(THIS, DOT)
				.sequence(typeNames).skipNested(LT, GT)
	...
	protected void createSubExpressionRules() {
		inState(IN_EXPRESSION)
				.sequence(NEW)
				.markStart()
				.repeated(IDENTIFIER, DOT)
				.sequence(IDENTIFIER)
				.createNode(EShallowEntityType.TYPE, "anonymous class",
						new Region(0, -1)).skipNested(LT, GT)
	...
	protected RecognizerBase<EGenericParserStates> getSubExpressionRecognizer() {
		return new JavaAnonymousClassRecognizer();
	}
}
