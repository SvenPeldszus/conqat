...
public class EntryEditor extends JPanel implements VetoableChangeListener {
	...
	public EntryEditor(JabRefFrame frame_, BasePanel panel_, BibtexEntry entry_) {
		...
		helpAction = new HelpAction(frame.helpDiag, GUIGlobals.entryEditorHelp,
				"Help");
		...
		if (prefs.getBoolean("showSource")
				&& prefs.getBoolean("defaultShowSource"))
	...
	private void setupFieldPanels() {
		...
		if ((entry.getOptionalFields() != null)
			...
			optPan = new EntryEditorTab(java.util.Arrays.asList(entry
					.getOptionalFields()), this, false,
		...
		for (int i = 0; i < tabList.getTabCount(); i++) {
			...
			tabbed.addTab(tabList.getTabName(i),
	...
	public JComponent getExtra(String string, FieldEditor editor) {
		...
		if ((s != null) && s.equals("external")) {
			...
			((JComponent) editor)
					.addMouseListener(new ExternalViewerListener());
			...
			if (panel.metaData.getData(Globals.SELECTOR_META_PREFIX
				...
				FieldContentSelector ws = new FieldContentSelector(frame,
						panel, frame, editor, panel.metaData, storeFieldAction,
						false, ", ");
				contentSelectors.add(ws);
				controls.add(ws, BorderLayout.NORTH);
			}
			controls.add(JournalAbbreviations.getNameSwitcher(this, editor,
					panel.undoManager), BorderLayout.SOUTH);
			return controls;
		} else if (panel.metaData.getData(Globals.SELECTOR_META_PREFIX
				+ editor.getFieldName()) != null) {
			FieldContentSelector ws = new FieldContentSelector(frame, panel,
					frame, editor, panel.metaData, storeFieldAction, false,
			...
			// } else if ((s != null) && s.equals("browsePdf")) {
				...
				&& (s.equals("browseDoc") || s.equals("browseDocZip"))) {

			final String ext = "." + fieldName.toLowerCase();
			final OpenFileFilter off;
			if (s.equals("browseDocZip"))
				off = new OpenFileFilter(new String[] { ext, ext + ".gz",
						ext + ".bz2" });
			else
				off = new OpenFileFilter(new String[] { ext });

			ExternalFilePanel pan = new ExternalFilePanel(frame,
					panel.metaData(), this, fieldName, off, ed);
			return pan;
		}
		/*
		 ...
		 */
			...
			((JComponent) editor).setDropTarget(new DropTarget(
					(Component) editor, DnDConstants.ACTION_NONE,
					new SimpleUrlDragDrop(editor, storeFieldAction)));
	...
	private void setupSourcePanel() {
		...
		source.setFont(new Font("Monospaced", Font.PLAIN, Globals.prefs
				.getInt("fontSize")));
	...
	/**
	 * NOTE: This method is only used for the source panel, not for the other
	 * tabs. Look at EntryEditorTab for the setup of text components in the
	 * other tabs.
	 */
	public void setupJTextComponent(JTextComponent ta) {
		...
		try {
			HashSet keys = new HashSet(
					ta.getFocusTraversalKeys(KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS));
			keys.clear();
			keys.add(AWTKeyStroke.getAWTKeyStroke("pressed TAB"));
			ta.setFocusTraversalKeys(
					KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS, keys);
			keys = new HashSet(
					ta.getFocusTraversalKeys(KeyboardFocusManager.BACKWARD_TRAVERSAL_KEYS));
			keys.clear();
	...
	public boolean storeSource(boolean showError) {
		...
		BibtexParser bp = new BibtexParser(new java.io.StringReader(
				source.getText()));
		...
		try {
			...
			NamedCompound compound = new NamedCompound(
					Globals.lang("source edit"));
			BibtexEntry nu = db.getEntryById((String) db.getKeySet().iterator()
					.next());
			...
			for (int i = 0; i < fields.length; i++) {
				if (BibtexFields.isDisplayableField(fields[i].toString())) {
					if (nu.getField(fields[i].toString()) == null) {
						compound.addEdit(new UndoableFieldChange(entry,
								fields[i].toString(), entry.getField(fields[i]
			...
			for (int i = 0; i < fields.length; i++) {
				if (entry.getField(fields[i].toString()) != nu
						.getField(fields[i].toString())) {
					...
					compound.addEdit(new UndoableFieldChange(entry, fields[i]
							.toString(), entry.getField(fields[i].toString()),
							toSet));
	...
	public void vetoableChange(PropertyChangeEvent e) {
		String newValue = ((e.getNewValue() != null) ? e.getNewValue()
				.toString() : "");
			...
			addMouseListener(new MouseAdapter() {
				public void mouseClicked(MouseEvent e) {
					...
					if ((e.getButton() == MouseEvent.BUTTON3)
						...
						for (Iterator i = BibtexEntryType.ALL_TYPES.keySet()
								.iterator(); i.hasNext();)
							typeMenu.add(new ChangeTypeAction(BibtexEntryType
									.getType((String) i.next()), panel));
	...
	class DeleteAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			panel.markBaseChanged();
			panel.undoManager.addEdit(new UndoableRemoveEntry(panel.database,
					entry, panel));
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() instanceof FieldTextArea) {
				...
				if (set) {
					try {
						...
						panel.undoManager.addEdit(new UndoableFieldChange(
								entry, fe.getFieldName(), oldValue, toSet));
						updateSource();
						panel.markBaseChanged();
						...
						JOptionPane.showMessageDialog(frame,
								"Error: " + ex.getMessage(),
								Globals.lang("Error setting field"),
				...
				if ((cleaned != null) && !cleaned.equals(newValue)) {
					JOptionPane.showMessageDialog(frame,
							Globals.lang("Invalid BibTeX key"),
							Globals.lang("Error setting field"),
							JOptionPane.ERROR_MESSAGE);
					fe.setBackground(GUIGlobals.invalidFieldBackground);
					return;
				...
				panel.undoManager.addEdit(new UndoableKeyChange(panel.database,
						entry.getId(), oldValue, newValue));
	...
	class SwitchLeftAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			tabbed.setSelectedIndex(((i > 0) ? (i - 1)
					: (tabbed.getTabCount() - 1)));
	...
	class SwitchRightAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			int i = tabbed.getSelectedIndex();
			tabbed.setSelectedIndex((i < (tabbed.getTabCount() - 1)) ? (i + 1)
					: 0);
			activateVisible();
	...
	class GenerateKeyAction extends AbstractAction {
		...
		public GenerateKeyAction(JabRefFrame parentFrame) {
			super(Globals.lang("Generate BibTeX key"), GUIGlobals
					.getImage("makeKey"));
			parent = parentFrame;
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				LabelPatternUtil.makeLabel(prefs.getKeyPattern(),
						panel.database, entry);

				// Store undo information:
				panel.undoManager.addEdit(new UndoableKeyChange(panel.database,
						entry.getId(), (String) oldValue, (String) entry
								.getField(BibtexFields.KEY_FIELD)));

				// here we update the field
				String bibtexKeyData = (String) entry
						.getField(BibtexFields.KEY_FIELD);
	...
	class ExternalViewerListener extends MouseAdapter {
		public void mouseClicked(MouseEvent evt) {
			if (evt.getClickCount() == 2) {
				...
				// getSelectedText()
				try {
					Util.openExternalViewer(panel.metaData(), link,
							tf.getFieldName());
				} catch (IOException ex) {
	...
	private boolean containedInExplicitGroup(BibtexEntry entry) {
		AbstractGroup[] matchingGroups = panel.getGroupSelector()
				.getGroupTreeRoot().getMatchingGroups(entry);
		...
		if (prefs.getBoolean("dialogWarningForDuplicateKey")) {
			...
			CheckBoxMessage jcb = new CheckBoxMessage(
					Globals.lang("Warning")
							+ ": "
							+ Globals
									.lang("Duplicate BibTeX key. Grouping may not work for this entry."),
		...
		if (prefs.getBoolean("dialogWarningForEmptyKey")) {
			...
			CheckBoxMessage jcb = new CheckBoxMessage(
					Globals.lang("Warning")
							+ ": "
							+ Globals
									.lang("Empty BibTeX key. Grouping may not work for this entry."),
					Globals.lang("Disable this warning dialog"), false);
			JOptionPane.showMessageDialog(frame, jcb, Globals.lang("Warning"),
					JOptionPane.WARNING_MESSAGE);
