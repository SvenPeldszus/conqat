 ...
 * @version $Rev: 47942 $
 * @ConQAT.Rating GREEN Hash: DF964A411FEB67E97A2FF80DC7816EDC
 */
public class UniformPathUtilsTest extends CCSMTestCaseBase {
	...
	/** Tests normalization of system-agnostic paths. */
	public void testCreateSystemIndependentPath() {
		assertEquals("foo/bar",
				UniformPathUtils.createSystemIndependentPath("C:/foo/bar"));
		assertEquals("foo/bar",
				UniformPathUtils.createSystemIndependentPath("C:\\foo\\bar"));
		assertEquals("foo/bar",
				UniformPathUtils.createSystemIndependentPath("c:\\foo\\bar"));
		assertEquals("foo/bar",
				UniformPathUtils.createSystemIndependentPath("/foo/bar"));
		assertEquals("foo/bar",
				UniformPathUtils.createSystemIndependentPath("foo/bar"));

		assertEquals("", UniformPathUtils.createSystemIndependentPath(""));
	}
}