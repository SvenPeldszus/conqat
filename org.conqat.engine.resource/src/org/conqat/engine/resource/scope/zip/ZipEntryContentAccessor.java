 ...
 * @author $Author: goeb $
 * @version $Rev: 51153 $
 * @ConQAT.Rating GREEN Hash: 3E413137E47F4239F45AFB931B860D13
 */
public class ZipEntryContentAccessor extends ContentAccessorBase {
	...
	/**
	 * The prefix, which is stripped from the uniform path. We need this one in
	...
	private static String stripEntryPrefix(CanonicalFile zipFile, String entry,
		...
		return StringUtils.stripPrefix(FileSystemUtils.normalizeSeparators(canonizedEntryName),
	...
	public IContentAccessor createRelative(String relativePath)
		...
		// conventions
		String newEntry = UniformPathUtils.concatenate(this.entryPrefix,
				createRelativePath(relativePath));
		try {
	...
	public String createRelativeUniformPath(String relativePath)
			throws ConQATException {
		String pathWithoutPrefix = createRelativePath(relativePath);
		String projectName = UniformPathUtils.extractProject(getUniformPath());

		return UniformPathUtils.concatenate(projectName, pathWithoutPrefix);
	}

	/** Creates a relative path to the entry prefix, without project name. */
	private String createRelativePath(String relativePath)
			throws ConQATException {
		// we can reuse the UniformPathUtils, as entries follow uniform path
		// conventions
		String path = UniformPathUtils.resolveRelativePath(entryName,
				UniformPathUtils.normalizeAllSeparators(relativePath));

		if (!path.startsWith(entryPrefix)) {
			throw new ConQATException("The relative path '" + relativePath
					+ "' is not located under the root of the uniform path.");
		}

		return stripEntryPrefix(this.zipFile, path, this.entryPrefix,
				this.caseSensitive);
	}
	...
	public String getLocation() {
		return zipFile.getCanonicalPath() + ZipPath.ZIP_FILE_SEPARATOR
				+ entryName;
	}
}