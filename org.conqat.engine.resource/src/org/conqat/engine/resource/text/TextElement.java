 ...
 * @version $Rev: 48257 $
 * @ConQAT.Rating GREEN Hash: 1385A3ED202C1386048EB472B742D7F0
 */
public class TextElement extends BinaryElement implements ITextElement {
	...
	public int convertUnfilteredLineToOffset(int line) throws ConQATException {
		LineOffsetConverter converter = filteredTextCache.obtain(
				new FilteredTextKey(this)).getUnfilteredLineOffsetConverter();
		String type = "unfiltered";
		checkValidLine(line, converter, type);
		return converter.getOffset(line);
	}

	/** Checks that the given line is valid in the line/offset converter */
	private void checkValidLine(int line, LineOffsetConverter converter,
			String contentType) throws ConQATException {
		if (!converter.isValidLine(line)) {
			throw new ConQATException("Failed while converting " + contentType
					+ " line " + line + " for " + getUniformPath()
					+ ", which has only " + converter.getLineCount() + " "
					+ contentType + " lines.");
		}
	}
	...
	public int convertFilteredLineToOffset(int line) throws ConQATException {
		LineOffsetConverter converter = filteredTextCache.obtain(
				new FilteredTextKey(this)).getFilteredLineOffsetConverter();
		checkValidLine(line, converter, "filtered");
		return converter.getOffset(line);
	}
