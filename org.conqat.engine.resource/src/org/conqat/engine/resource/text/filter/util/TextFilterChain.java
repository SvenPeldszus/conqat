 ...
 * @version $Rev: 50092 $
 * @ConQAT.Rating GREEN Hash: C85899D119A088285B857CFDE76D8C5D
 */
public class TextFilterChain extends ArrayList<ITextFilter> implements
	...
	public List<Deletion> getDeletions(String string, String originUniformPath)
		...
		// create new array list, as returned list might be unmodifiable
		List<Deletion> deletions = new ArrayList<>(get(0).getDeletions(string,
				originUniformPath));
		for (int i = 1; i < size(); ++i) {
			StringOffsetTransformer transformer = new StringOffsetTransformer(
					deletions);
			String filteredString = transformer.filterString(string);
			List<Deletion> filteredDeletions = get(i).getDeletions(
					filteredString, originUniformPath);
