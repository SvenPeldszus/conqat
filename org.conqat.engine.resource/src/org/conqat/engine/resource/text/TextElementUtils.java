 ...
 * @version $Rev: 49008 $
 * @ConQAT.Rating GREEN Hash: EE9CFD3F88C25137AB5225CF72596C78
 */
public class TextElementUtils {
	 ...
	 * filters are applied.
	 */
	public static void parseSAX(ITextElement element, DefaultHandler saxHandler)
			throws ConQATException {
		CanonicalFile underlyingFile = ResourceUtils.getFile(element);
		try {
			if (underlyingFile != null) {
				...
				// file in memory
				XMLUtils.parseSAX(
						XMLUtils.toInputSource(element.getTextContent()),
						saxHandler);
			}
		} catch (SAXException | IOException e) {
			throw new ConQATException("Error parsing file "
					+ element.getLocation(), e);
		}
	}
}
