 ...
 * @author $Author: goeb $
 * @version $Rev: 51603 $
 * @ConQAT.Rating GREEN Hash: 4E0A37389B8CCBA73071877B5A4EF39F
 */
public class TreeMapImageDescriptor extends ImageDescriptorBase {
	...
	/**
	 * Flag that determines whether or not to use the squarified layout
	 * algorithm.
	 */
	private final boolean useSquarifiedAlgorithm;
	 ...
	 * @param drawLegend
	 *            whether to draw a legend under the treemap.
	 * @param useSquarifiedAlgorithm
	 *            whether to use the squarified layouting algorithm or the
	 *            normal striped algorithm.
	 */
	public TreeMapImageDescriptor(double cushionHeight, double cushionScale,
			...
			Object summary, boolean drawLegend, boolean useSquarifiedAlgorithm) {
		super("tree_map.gif");
		...
		this.useSquarifiedAlgorithm = useSquarifiedAlgorithm;
	}
	...
	public void draw(Graphics2D graphics, int width, int height) {
		...
		getLayoutAlgorithm().layout(treeMapRoot,
				new Rectangle(0, 0, width, actualHeight));
	...
	/** Return the layout algorithm to use. */
	private ITreeMapLayoutAlgorithm getLayoutAlgorithm() {
		if (useSquarifiedAlgorithm) {
			return new SquarifiedTreeMapAlgorithm();
		}
		return new StripeTreeMapAlgorithm();
	}
	...
	private void drawTopLevelFrame(Graphics2D graphics) {
		...
		for (ITreeMapNode<IConQATNode> child : node.getChildren()) {
			Rectangle2D rectangle = child.getLayoutRectangle();
			if (rectangle != null) {
	...
	private static class TooltipDescriptor implements
		...
		private TooltipDescriptor(ITreeMapNode<IConQATNode> treeMapRoot,
				int width, int height, ITreeMapLayoutAlgorithm algorithm) {
			this.treeMapRoot = treeMapRoot;
			algorithm.layout(treeMapRoot, new Rectangle(0, 0, width, height));
		}
