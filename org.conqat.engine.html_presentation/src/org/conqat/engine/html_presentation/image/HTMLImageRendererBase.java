...
import static org.conqat.engine.commons.css.CSSMananger.IMAGE_STYLE;
...
/**
 * Base class for processors rendering images. For rendering images the
 * {@link #addImageDescriptor(IImageDescriptor, String)} has to be called by
 * implementing classes. If more than one image descriptor is provided, all
 * information (such as tooltips) are based on the first image and links to
 * switch between the images are created.
 * 
 * @author $Author: kinnen $
 * @version $Rev: 49983 $
 * @ConQAT.Rating GREEN Hash: BBEBFA3A56A12F2BEE176D0942A07168
 */
public abstract class HTMLImageRendererBase extends ImageCreatingLayouterBase {

	/** The image descriptors and names. */
	private final PairList<IImageDescriptor, String> descriptors = new PairList<IImageDescriptor, String>();
	...
	/** Adds an image descriptor to render. */
	protected void addImageDescriptor(IImageDescriptor descriptor, String name) {
	...
	protected void layoutPage() throws ConQATException {
		...
		Dimension dimension = EImageFormat.PNG.writeImage(file,
				descriptors.getFirst(0), width, height);
		...
		String id = BundleContext.getInstance().getHtmlPresentationManager()
	...
	private HTMLImageMapGenerator getImageMapGenerator(Dimension dimension)
			throws ConQATException {
		ITooltipDescriptor<Object> tooltipDescriptor = descriptors.getFirst(0)
				.getTooltipDescriptor(dimension.width, dimension.height);
	...
	/** Checks if at least one descriptor is present. */
	private void assertDescriptors() throws AssertionError {
		CCSMAssert.isFalse(descriptors.isEmpty(),
				"Contract violated. No descriptors added by subclass "
						+ getClass());
	}
}