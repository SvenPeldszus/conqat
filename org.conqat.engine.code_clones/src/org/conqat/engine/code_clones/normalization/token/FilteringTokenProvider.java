 ...
 * @version $Revision: 49546 $
 * @ConQAT.Rating GREEN Hash: C55810BC5FDBE11F92DD5F650EC3E5C5
 */
public class FilteringTokenProvider extends TokenProviderBase implements
	...
	protected IToken provideNext() throws CloneDetectionException {
		...
		if (lastToken != null && token != null
				...
				&& !isSynthetic(token) && gapBefore(token)) {
	...
	private boolean ignoredRegionStarts(IToken token) {
		if (lastToken != null && token != null && !isSynthetic(token)) {
	...
	/** Returns whether a token is synthetic */
	public boolean isSynthetic(IToken token) {
		return token.getType().getTokenClass() == ETokenClass.SYNTHETIC;
	}
