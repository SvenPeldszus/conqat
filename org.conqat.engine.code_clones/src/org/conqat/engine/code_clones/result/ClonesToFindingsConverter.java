 ...
 * @version $Rev: 47530 $
 * @ConQAT.Rating YELLOW Hash: A1C3FED8AF916A58CDDEB77103A3F332
 */
...
public class ClonesToFindingsConverter extends DetectionResultProcessorBase {
	...
	/**
	 * Finding category name for clone findings.
	 */
	public static final String CLONE_FINDING_CATEGORY = "Clones";
	...
	public CloneDetectionResultElement process() {
		category = NodeUtils.getFindingReport(detectionResult.getRoot())
				.getOrCreateCategory(CLONE_FINDING_CATEGORY);
	...
	private void convertClonesToFindings() {
		for (CloneClass cloneClass : detectionResult.getList()) {
			...
			if (includeFingerprints) {
				group.setValue(EFindingKeys.FINGERPRINT.name(),
						cloneClass.getFingerprint());
			}
			for (Clone clone : cloneClass.getClones()) {
				...
				Finding finding = FindingUtils.createAndAttachFinding(group,
						"Clone (class-id " + clone.getCloneClass().getId()
								+ ") of length " + clone.getLengthInUnits(),
						element, clone.getLocation(), key);
