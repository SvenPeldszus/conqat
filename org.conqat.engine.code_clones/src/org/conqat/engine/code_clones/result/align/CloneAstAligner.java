 ...
 * @version $Rev: 48881 $
 * @ConQAT.Rating GREEN Hash: 82C4F64BD4FDEDF65FD6B09E48CAEF91
 */
...
public class CloneAstAligner extends DetectionResultProcessorBase {
	 ...
	 * list.
	 */
	/* package */static void insertStatements(List<ShallowEntity> entities,
			int depth, ITokenElement element, List<AlignerStatement> statements)
	 ...
	 * list.
	 */
	private static void insertStatements(ShallowEntity entity, int depth,
			ITokenElement element, List<AlignerStatement> statements)
			throws ConQATException {
		// ignore defines, annotations, imports, etc., as well as empty
		// fragments resulting from parser bugs
		...
		statements.add(new AlignerStatement(entity.ownStartTokens(), element,
				depth, true, EAlignerStatementType.COMPOUND_START));

		insertStatements(entity.getChildren(), depth + 1, element, statements);

		statements.add(new AlignerStatement(entity.ownEndTokens(), element,
				depth, true, EAlignerStatementType.COMPOUND_END));
	}
}
