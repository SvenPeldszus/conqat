 ...
 * @version $Rev: 48006 $
 * @ConQAT.Rating YELLOW Hash: 4F8117206D1F837C593FFB00BA303E47
 */
...
public class SimilarityConstraint extends UnitsConstraintBase {
	...
	public boolean satisfied(CloneClass cloneClass) throws ConQATException {
		...
		for (Clone clone : cloneClass.getClones()) {
			List<String> unnormalizedTokenContent = getUnnormalizedUnitContent(clone);
			if (length == -1) {
	...
	private List<String> getUnnormalizedUnitContent(Clone clone)
			throws ConQATException {
		List<String> statementTexts = new ArrayList<String>();

		for (Unit unit : getUnits(clone)) {
			if (unit instanceof StatementUnit) {
				StatementUnit statement = (StatementUnit) unit;
				extractUnnormalizedContent(statement, statementTexts);
			} else {
	...
	/**
	 * Extracts the unnormalized content from the given statement and adds it to
	 * the list of texts.
	 */
	protected void extractUnnormalizedContent(StatementUnit statement,
		...
		for (TokenUnit tokenUnit : statement.getTokens()) {
			...
			// we ignore casing differences here, since some
