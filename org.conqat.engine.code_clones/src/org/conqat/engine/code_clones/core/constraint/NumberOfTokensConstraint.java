 ...
 * @version $Rev: 48003 $
 * @ConQAT.Rating YELLOW Hash: 773D3C503DB34E43554864D229D31FF3
 */
@AConQATProcessor(description = "Constraint that is satisfied if the statements of a clone contain at least a "
		+ "minimal number of tokens. Only applicable if units are stored. If not all clones in a clone class contain "
...
public class NumberOfTokensConstraint extends UnitsConstraintBase {
	...
	private int numberOfTokens(Clone clone) throws ConQATException {
		...
		for (Unit unit : getUnits(clone)) {
			if (unit instanceof StatementUnit) {
				tokenCount += ((StatementUnit) unit).getTokens().length;
