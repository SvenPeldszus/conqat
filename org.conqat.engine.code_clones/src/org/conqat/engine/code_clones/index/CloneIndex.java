 ...
 * @version $Rev: 48915 $
 * @ConQAT.Rating GREEN Hash: C82727A60A670640897613DE5C6B09C1
 */
public class CloneIndex {
	...
	public boolean reportClones(String originId, ICloneClassReporter reporter,
		...
		if (allowGaps) {
			List<ChunkList> orderedChunks = ChunkUtils.obtainOrderedChunks(
					store, chunks);
			...
			List<Chunk> matchingChunks = ChunkUtils.getMatchingChunks(store,
					chunks);
			...
			new CloneIndexCloneSearcher(originId, reporter, onlyStartingHere,
					minLength, matchingChunks, options.getChunkLength())
					.reportClones();
		}
