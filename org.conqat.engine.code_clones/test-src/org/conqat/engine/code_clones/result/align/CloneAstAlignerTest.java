 ...
 * @version $Rev: 48881 $
 * @ConQAT.Rating GREEN Hash: 096E6E43F19541F30580854A56BD9DBF
 */
...
public class CloneAstAlignerTest extends TokenTestCaseBase {
	...
	/**
	 * Test case for CR#6281. This is very specific, as currently only the
	 * JavaScript parser does create these empty entities and also only for
	 * invalid JavaScript.
	 */
	public void testStatementInsertionForBrokenParse() throws Exception {
		ITokenElement element = createTokenElement("var x; }();",
				ELanguage.JAVASCRIPT);
		List<ShallowEntity> entities = ShallowParserFactory.createParser(
				ELanguage.JAVASCRIPT).parseTopLevel(
				element.getTokens(new LoggerMock()));

		List<AlignerStatement> statements = new ArrayList<>();
		CloneAstAligner.insertStatements(entities, 0, element, statements);
