 ...
 * @version $Rev: 50784 $
 * @ConQAT.Rating GREEN Hash: 2454CC866C51BB9EF94BC4DD2B4AEA77
 */
	...
	private final class BugCollectionReader
		...
		private String getLocation(String relativeFile) throws ConQATException {
			...
			if (result == null) {
				for (String srcDir : srcDirs) {
					...
					if (elementResolver.isValidLocation(path)) {
						result = path;
						break;
					}
				}
				...
				if (result == null) {
					String message = "Could not resolve relative file "
							+ relativeFile
							+ "! Probably your FindBugs report has incorrect srcDir information or the files have moved.";
					handleError(message);
				} else {
		...
		private final class BugInstanceProcessor implements
			...
			public void process() throws ConQATException {
				...
				extractLocationInformation(cp, classNames, methodAndFieldNames);
			...
			/**
			 * Extracts location information from the child elements and stores
			 * it in the provided collections.
			 */
			private void extractLocationInformation(ClassProcessor cp,
		...
		private final class SourceLineProcessor implements
			...
			public void process() throws ConQATException {
				...
				if (startLine != null && endLine != null) {
					...
					String location = getLocation(sourcePath);
					if (location != null) {
						if (createLineRegionFinding(findbugsRule, message,
								location, startLine, endLine) != null) {
							findingCreated = true;
						}
					}
				}
