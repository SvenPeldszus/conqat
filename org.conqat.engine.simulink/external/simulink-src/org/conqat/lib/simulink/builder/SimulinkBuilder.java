 ...
 * @version $Rev: 51491 $
 * @ConQAT.Rating GREEN Hash: ACE9F6783F1E9DD423E1F1412D6629B8
 * 
...
/* package */class SimulinkBuilder {
	...
	/** Create new builder. */
	public SimulinkBuilder(SimulinkModel model,
			ModelBuildingParameters parameters, boolean isSlx) {
		this.model = model;
	...
	/** Build objects. */
	private void buildObjects(MDLSection section,
			SimulinkElementBase simulinkElement) {
	...
	private void buildBlockTypeDefaultParams(MDLSection modelSection) {
		...
		for (String name : blockDefaults.getParameterNames()) {
			model.setBlockDefaultParameter(name,
					blockDefaults.getParameter(name));
		}
	}
		...
		for (MDLSection childBlock : blockParameterDefaults
			...
			for (String name : childBlock.getParameterNames()) {
				...
				if (!name.equals(PARAM_BlockType)) {
					model.setBlockTypeDefaultParameter(type, name,
							childBlock.getParameter(name));
				}
			}
		}
	}
	...
	private void buildAnnotationDefaultParams(MDLSection modelSection) {
		...
		for (String name : annotationDefaults.getParameterNames()) {
			model.setAnnotationDefaultParameter(name,
					annotationDefaults.getParameter(name));
		}
	}
