 ...
 * @version $Rev: 49870 $
 * @ConQAT.Rating GREEN Hash: 18C6EE4CB99628A8239667C8B368C88B
 */
...
public class SimulinkBlockParameterAssessor extends
		PatternBasedBlockTraversingProcessorBase {
	...
	/**
	 * This map from (Block type X parameter name) to rules. Implementation
	 * would be way more elegant if there was pattern that match no string at
	 * all.
	 */
	private final HashMap<String, List<Rule>> patterns = new HashMap<String, List<Rule>>();

	/** {@ConQAT.Doc} */
	@AConQATParameter(name = "rule", description = "Add rule to be checked by the processor.", minOccurrences = 1)
	public void addRule(
			@AConQATAttribute(name = "name", defaultValue = NONE, description = "The name of the rule") String name,
		...
		List<Rule> ruleList = patterns.get(blockType);
		...
		Pattern conditionPattern = createPattern(conditionPatternString,
				ALLOW_EVERYTHING_PATTERN);

		ruleList.add(new Rule(name, conditionParameter, conditionPattern,
				paramMustBePresent, parameterName, allowPattern, denyPattern));
	}
		...
		for (Rule rule : rules) {
			if (checkCondition(rule, block)) {
				if (rule.paramMustBePresent
					...
					attachFinding(String.format(
							"Rule '%s': Parameter %s not present", rule.name,
	...
	private class Rule {
		...
		/**
		 * Flag that signals if the parameter (not the condition parameter) must
		 * be present
		 */
		...
		/** constructor */
		public Rule(String name, String conditionParameter,
				Pattern conditionPattern, boolean paramMustBePresent,
				String param, Pattern allowPattern, Pattern denyPattern) {
			super();
			this.name = name;
